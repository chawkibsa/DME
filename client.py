# Client side ADVANCED GUI chatroom
...
# Define window
root = tkinter.Tk()
root.title ("DMEE (User)")
root.iconbitmap("./images/DME.ico")
...
class Connection():
    '''A class to store a connection - a client socket and a pertinent information'''
    def __init__(self):
        self.ENCODER = "utf-8"
        self.BYTESIZE = 1024
...
def connect(connection):
    ...
    # Get required information for connction from input fields
    ...
    try:
        ...
        my_listbox.insert(0,f"Connection not established ... Goodbye. the error is\n {msg}")
...
def process_message(connection, message_json):
    ...
    if flag == "INFO":
        ...
        # Create a thread to continiously recieve messages from the server
        recieve_thread = threading.Thread(target=recieve_message, args=(connection,))
        recieve_thread.start()
    elif flag == "MESSAGE":
        # Server has send a message so display it
...
def recieve_message(connection):
    ...
    while True:
        # Recieve an incoming message packet from the server
        try:
            # Recieve an incoming message packet
            message_json = connection.client_socket.recv(connection.BYTESIZE)
            process_message(connection, message_json)
        except:
            # Cannot recieve message, close the connection and break
